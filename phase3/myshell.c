...
int addjob(pid_t pid,char* cmdline){
    JOB* last = tail->prev;
    JOB* newjob = (JOB*)malloc(sizeof(JOB));
    newjob->jid = last->jid + 1;
    newjob->pid = pid;
    strcpy(newjob->cmd, cmdline);
    newjob->state = 1;
...
pid_t findjobpid(int jobno){
    JOB* cur = head->next;
    while(cur->jid != jobno && cur != tail){
        cur = cur->next;
    }
    return cur->pid;
...
void sigchld_handler(int sig){
    ...
    pid_t pid;
    Sigfillset(&mask_all);
    while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0){ /* children(more than one) can be waiting to be reaped in WAIT set */
...
void pipe_line(char *cmdline){
    ...
    cmd = (char**)malloc(sizeof(char*)*strlen(cmdline));
    for(int i=0;i<strlen(cmdline);i++)
        cmd[i] = (char*)malloc(sizeof(char)*strlen(cmdline));
    
    char* temp = strtok(cmdline,"|");
    strcpy(cmd[cmd_num++],temp);
        ...
        if(ppid == 0){
            for(int i=0;i< cmd_num;i++){
                ...
                if(pid == 0){
                    ...
                    if(i+1 != cmd_num) dup2(fd[1],STDOUT_FILENO);
                    close(fd[0]);       
                    eval(cmd[i]);            
                    exit(0);
                }
...
int builtin_command(char **argv) 
{
    if (!strncmp(argv[0], "quit",4)) /* quit command */
	    exit(0);
    if (!strncmp(argv[0], "exit",4)) /* quit command */
	    exit(0);      
    if (!strncmp(argv[0], "&",1))    /* Ignore singleton & */
	    return 1;
    if (!strncmp(argv[0], "cd",2)){
        char* buf[MAXARGS];
        parseline(argv[0], buf);
        chdir(buf[1]);
        return 1;
    }
    ...
    if (!strncmp(argv[0], "kill",4)){
        parseline(argv[0],argv);
        char* delim;
        if(delim = strchr(argv[1],'%')){
            argv[1] = delim+1;
            int job_pid = findjobpid(atoi(argv[1]));
            if(job_pid)
